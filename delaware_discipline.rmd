...
Now we can start calculating. First, the population
```{r}
# This makes a table showing the number of students of x race enrolled each year.
pops <- delaware_pop |> 
    filter(district=="State of Delaware") |> 
    filter(grade !=(regex("Birth to 2 Years", ignore_case=TRUE))) |> 
    select(school_year, grade, race, students, special_demo, sub_group, gender)
view(pops)

pop_hisp <- pops|> 
    filter(race==name_hisp) |> 
    filter(sub_group==name_hisp) |> 
    summarize(sum=sum(students))
pop_native_am <- pops|> 
...
pop_hawaiian_pi <- pops|> 
    ...
    summarize(sum=sum(students)) 
pop_stats <- as.data.frame(c(pop_hisp, pop_native_am, pop_black, pop_white, pop_asian_am, pop_multi_racial, pop_hawaiian_pi))
colnames(pop_stats) <- c(name_hisp, name_native_am, name_black, name_white, name_asian_am, name_multi_racial, name_hawaiian_pi)
view(pop_stats)

pops_per_grade <- pops |> 
    filter(race=="All Students") |> 
    filter(gender=="All Students") |> 
    filter(special_demo=="All Students") |> 
    select(school_year, grade, students)
view(pops_per_grade)

pops_per_grade_py <- statewide_only_discipline |> 
    filter(race=="All Students") |> 
    filter(category=="In-School Suspension") |> 
    filter(gender=="All Students") |> 
    filter(special_demo=="All Students") |> 
    filter(grade!="All Students") |> 
    select(school_year, grade, enrollment) |> 
    group_by(grade) |> 
    summarize(sum=sum(enrollment))
view(pops_per_grade)

pops_per_grade_race <- statewide_only_discipline |> 
    filter(race!="All Students") |> 
    filter(category=="In-School Suspension") |> 
    filter(gender=="All Students") |> 
    filter(special_demo=="All Students") |> 
    ...
    group_by(grade) |> 
    summarize(sum=sum(enrollment))
view(pops_per_grade_race)


# before we do this let's double check something, whether male and female equals the number of all students
experiment <- statewide_only_discipline |> 
    filter(race==name_black &
    school_year==2015 &
    str_detect(grade, "1st") &
    gender != "All Students") |> 
    group_by(gender) |> 
    summarize(sum(enrollment=sum(na.omit(enrollment))))
...
# this is important for when we want to calculate PER GRADE
filtered_df <- statewide_only_discipline %>%
  filter(str_detect(sub_group, regex("\\d{1,2}(?:st|nd|rd|th) grade$", ignore_case = TRUE))) |> 

pop_hisp_10th |> 
    filter(sub_group == "10th Grade") |> 
    filter(race==name_hisp) |> 
